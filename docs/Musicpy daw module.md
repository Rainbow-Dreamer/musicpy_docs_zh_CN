...
在musicpy文件夹中的`daw.py`文件中，有一个叫做`Synth`的类，它是一个标准的接口，可以用来制作一个可以在daw对象中使用的python乐器或效果器。
...
要清除宿主对象的一个通道，使用`clear_channel`方法，索引是基于0的。(清除一个通道意味着重置该通道的名称、音源和其他设置的属性)
