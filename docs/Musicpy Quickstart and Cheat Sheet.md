...
注意：muscipy的和弦类型有一个`interval`属性，它是和弦的每两个相邻的音符开始之间的小节长度的列表，请不要把它与乐理中的音程混淆，后者是两个音符之间的音高差异（例如大三度，半音）。在速查表中，我们用 `音程`来表示乐理中的音程。
...
| 获取和弦A的第n个转位 | A / n | A.inversion(n) |
| 将和弦A的第n个音转换为最高音 | A ^ n | A / -n; A.inversion_highest(n) |
|将和弦A的第n个音反转为最低音 | A @ n | A.inv(n) |
|       从和弦A的音符中按照索引组成一个新的和弦（基于1）       | A @ [1, 2, 3, 2, 1.1, 2, 3, 2] ||
|从和弦A的音符中按照索引组成一个新的和弦（基于1），增加八度数| A / [1, 2, 4, 3] ||
...
| 提高/降低作品A n个半音 | A + n; A - n | |
| 提高/降低作品A的第1个音轨n个半音 | A[i] += n; A[i] -= n | |
| 重复乐曲A n次 | A * n | |
| 将片断A和片断B连接起来 | A + B | A \| B |
| 堆叠片A和片B | A & B | |
| 删除A片的第1个轨迹 | del A[i] | |
| 获取A片的反向数据 | ~A | A.reverse() |
| 获得A片的轨道数 | len(A) | |
| 在A片上添加一个新的轨道n | A.append(n) | |
|在A片的索引i处插入一个新的音轨n | A.insert(i, n) | |
| 获取乐曲A的总长度（小节） | A.bar() | |
...
| 构建一个音量实例，数值为100%，开始时间为0 | volume(100, start_time=0) | |
| | | |
| | | |
| | | |
