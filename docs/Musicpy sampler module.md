...
### 左右声道混音位置和通道的音量
一个乐曲类型（或音轨类型）的通道的左右声道混音位置和音量也适用于musicpy取样机, 它们的作用与输出的MIDI文件相同。
...
```
生成的基本波形的和弦类型可以作为常规的musicpy数据结构用于取样机对象的播放/导出功能中作为常规的musicpy数据结构使用，并且可以放在一个乐曲类型（或一个音轨类型）中，由取样机对象播放/导出。
