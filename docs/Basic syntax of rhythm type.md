...
节奏实例的`total_bar_length`属性是与节奏实例的拍号相对应的小节的总长度，例如，如果一个节奏实例包含3个持续时间为1/4的拍子实例，拍号为3/4，那么节奏实例的总长度为1。

节奏实例的`time_signature`属性是节奏实例的拍号，其值是一个列表`[分子，分母]`。

你可以使用`get_length`方法来获取节奏实例的总拍子数，包括休止符和延续符，并应用点号。你也可以指定参数`beat_list`来获得一个节拍列表的总节拍数。
...
* chords: 可以是一个和弦实例，一个和弦列表或一个音符实例，如果是一个音符实例，结果将是一个和弦实例，音符与节奏重复
...
* include_continue: 是否考虑音符的持续时间来转换为持续符
